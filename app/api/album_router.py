...
async def upload_get(request: Request, db: Session = Depends(get_db)):
    ...
    return templates.TemplateResponse(
        "album_list.html", {"request": request, "token": user.email, "album_list": album_list}
    )
...
async def modify_name(
    ...
    if check_code == "edit":
        upload_info = (
            ...
            .filter((models.Upload.name == origin_name) & (models.Upload.upload_id == upload_id))
            .first()
        )
...
async def upload_get_one(
    request: Request, user_id: int = Query(...), upload_id: int = Query(...), db: Session = Depends(get_db)
):
    ...
    video_obj = s3.generate_presigned_url(
        "get_object", Params={"Bucket": settings.BUCKET, "Key": video.video_url}, ExpiresIn=3600
    )
    ...
    if not video_info["complete"]:
        return templates.TemplateResponse(
            "album_detail.html",
            {"request": request, "token": user.email, "video_info": video_info, "loading": True},
        )

    if frame_ids != []:
        for frame_id, frame_url, frame_timestamp in zip(frame_ids, frame_urls, frame_timestamps):
            frame_obj = s3.generate_presigned_url(
                "get_object", Params={"Bucket": settings.BUCKET, "Key": frame_url}, ExpiresIn=3600
            )
        ...
        score_obj = s3.generate_presigned_url(
            "get_object", Params={"Bucket": settings.BUCKET, "Key": score_graph_url}, ExpiresIn=3600
        )
    ...
    return templates.TemplateResponse(
        "album_detail.html",
        {"request": request, "token": user.email, "video_info": video_info, "loading": False},
    )


@router.get("/details/images")
async def image_get(request: Request, frame_id: int = Query(...), db: Session = Depends(get_db)):

    user = get_current_user(request)
    frame = crud.get_frame(db=db, frame_id=frame_id)
    frame_obj = s3.generate_presigned_url(
        "get_object", Params={"Bucket": settings.BUCKET, "Key": frame.frame_url}, ExpiresIn=3600
    )
    print(frame_obj)
    print(frame.box_kp_json)
    frame_info = {"frame_url": frame_obj, "time_stamp": frame.time_stamp, "frame_json": frame.box_kp_json}

    return templates.TemplateResponse(
        "frame.html", {"request": request, "token": user.email, "frame_info": frame_info}
    )
...
def main():
    ...
    ## 정상구간에서 mse 점수 분포
    print('mae')
    print("평균[{:.6f}]\n중간[{:.6f}]\n최소[{:.6f}]\n최대[{:.6f}]".format(np.mean(loss_list), np.median(loss_list), np.min(loss_list), np.max(loss_list)))
    print('=' * 40)
    print('mse')
    print("평균[{:.6f}]\n중간[{:.6f}]\n최소[{:.6f}]\n최대[{:.6f}]".format(np.mean(loss_m_list), np.median(loss_m_list), np.min(loss_m_list), np.max(loss_m_list)))
    print('=' * 40)
